...
//END SETTING
    cv::Mat drawing = Mat::zeros(blackroi.rows + 2,blackroi.cols + 2, CV_8UC1);
    //Function doing all the morphological operations to find the blobs
    contours = morphologicalOps(blackroi,contours,drawing);
    vector<Rect> boundRect( contours.size() );
    newDetected.resize(contours.size());
    //Function setting values to new Detected blobs
    newDetected = setValuesNew(boundRect, contours, roi, newDetected, &newCroppedImages,rng);
 ...
 for (unsigned int i = 0; i < Tracked.size(); ++i){
      ...
      if(boundRect.size() > 0){
        ...
        rectangle( frame, Tracked[i].getTl() + Point(0,0.4*frame.rows), Tracked[i].getBr() + Point(0,0.4*frame.rows), Tracked[i].getRectColor(), 2, 8, 0 );
      	stringstream ss;
      	...
      	string str = ss.str();
      	putText(frame,str,Tracked[i].getTl()+ Point(0,0.4*frame.rows), FONT_HERSHEY_SIMPLEX,1,Tracked[i].getRectColor(),3);
      }
    }
    //END Drawing
